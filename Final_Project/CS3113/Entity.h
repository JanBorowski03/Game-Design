...
private:
    ...
    bool mAnimateWhenIdle = false;
    bool mFlipOnLeftDirection = false;
    bool mGroundShadowEnabled = false;
    Vector2 mGroundShadowOffset = { 0.0f, 0.0f };
    float mGroundShadowOffsetYFactor = 0.34f;
    float mGroundShadowWidthFactor = 0.88f;
    float mGroundShadowHeightFactor = 0.30f;
    float mGroundShadowSoftness = 0.35f;
    float mGroundShadowAlpha = 0.40f;
...
public:
    ...
    void setAnimateWhenIdle(bool shouldAnimateWhenIdle)
        { mAnimateWhenIdle = shouldAnimateWhenIdle; }
    void setFlipOnLeftDirection(bool shouldFlipOnLeftDirection)
        { mFlipOnLeftDirection = shouldFlipOnLeftDirection; }
    void setGroundShadowEnabled(bool enabled)
        { mGroundShadowEnabled = enabled; }
    void setGroundShadowOffset(Vector2 offset)
        { mGroundShadowOffset = offset; }
    void setGroundShadowStyle(
        float offsetYFactor,
        float widthFactor,
        float heightFactor,
        float softness,
    ...
    {
        mGroundShadowOffsetYFactor = offsetYFactor;
        mGroundShadowWidthFactor = widthFactor;
        mGroundShadowHeightFactor = heightFactor;
        mGroundShadowSoftness = softness;
        mGroundShadowAlpha = alpha;
    }
    void setAnimationIndices(const std::vector<int> &newAnimationIndices)
    {
        mAnimationIndices = newAnimationIndices;
        if (mAnimationIndices.empty() || mCurrentFrameIndex >= static_cast<int>(mAnimationIndices.size()))
        {
