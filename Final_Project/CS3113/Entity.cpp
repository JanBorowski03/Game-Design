...
Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath) : mPosition {position}, mVelocity {0.0f, 0.0f}, 
...
void Entity::renderGroundShadow()
{
    if (!mGroundShadowEnabled) return;

    const Rectangle shadowDest = {
        mPosition.x + mGroundShadowOffset.x,
        mPosition.y + (mScale.y * mGroundShadowOffsetYFactor) + mGroundShadowOffset.y,
        mScale.x * mGroundShadowWidthFactor,
        mScale.y * mGroundShadowHeightFactor
    };
    Effects::renderEffect(UNIT_SHADOW, shadowDest, mGroundShadowSoftness, mGroundShadowAlpha);
}
    ...
    Entity *collidableEntities, int collisionCheckCount)
{
    ...
    if (mTextureType == ATLAS && (GetLength(mMovement) != 0 || mAnimateWhenIdle))
        animate(deltaTime);
}
...
void Entity::render()
{
    ...
    if (mFlipOnLeftDirection && mDirection == LEFT)
    {
