...
void processInput() 
{
    ...
    if(isColliding(&gMousePosition, &MOUSE_SIZE, &gTeardropPosition, &TEARDROP_BASE_SIZE) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) gTeardropStatus = RELEASED;
...
void update() 
{
    ...
    if(gTeardropPosition.y > SCREEN_HEIGHT + TEARDROP_BASE_SIZE.y){
        gTeardropStatus = HANGING;
        gTeardropPosition = TEARDROP_INIT_POS;
        gScale = TEARDROP_BASE_SIZE;
        gPreviousTime = GetTime();
    }

    // TODO - Stop horizontal translation once teardrop is released
    if(gTeardropStatus == HANGING) gTeardropPosition.x = gTeardropPosition.x + SPEED * cos(GetTime() - gPreviousTime) * deltaTime; // moving back and forth on the X-AXIS

    // TODO - Add vertical translation for teardrop
    else if(gTeardropStatus == RELEASED) gTeardropPosition.y = gTeardropPosition.y + SPEED * deltaTime; 

    // TODO - If teardrop is colliding with beaker, shrink the size of teardrop
    //        in a way that is FRAME-INDEPENDENT
    if(isColliding(&gTeardropPosition, &TEARDROP_BASE_SIZE, &BEAKER_INIT_POS, &BEAKER_BASE_SIZE)){
