...
void update() 
{
    ...
    gScale = {
        BASE_SIZE.x + MAX_AMP * cos(gTime),
        BASE_SIZE.y + MAX_AMP * cos(gTime)
    };

    /**
     * @todo Apply delta time to the time-dependent logic
     */
    gPulseTime += 1.0 * deltaTime;
    ...
    gCurrentMember = int(gPulseTime * FPS / 100) % 4;
}
    ...
    switch (gCurrentMember){
        case MURDOC :
            textureArea = {
                ...
                static_cast<float>(gTeardropTexture.width/2),
                static_cast<float>(gTeardropTexture.height/2)
            };
            break;
